...
const EditorPage: FC = () => {
	...
	const [textObjects, setTextObjects] = useState<{ [key: string]: FabricText }>(
		{}
	); // Fabric Text object for text on canvas
	...
	// 		fontStyle: isItalic ? 'italic' : 'normal',
	// 	});
	// 	fabricText.set('id', id); // Set the id property
	...
	// 	if (canvas) {
	// 		const textObject = textObjects[textId]; // Retrieve the text object by ID
	// 		if (textObject) {
	// 			textObject.set({
	// 				text,
	// 				fontSize,
			...
			<div className='w-1/4 p-6 bg-app_bg_secondary h-full overflow-y-auto no-scrollbar'>
				...
				<div className='mb-6 flex space-x-5 items-center'>
					...
					<input
						type='color'
						value={rectColor}
						onChange={(color) => setRectColor(color.target.value)}
					/>
				</div>
